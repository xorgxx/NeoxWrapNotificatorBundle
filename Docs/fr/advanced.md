...
Le bundle peut publier les messages flash Symfony via Mercure au moment de `kernel.response`, afin de les afficher instantanément dans le navigateur (toasts), sans rechargement de page.
...
Par défaut, chaque message flash est publié et affiché comme une notification distincte.
...
### Activation ciblée via attribut

Vous pouvez activer/désactiver localement via l’attribut `#[LiveFlash]` (classe ou méthode) :
...
### Topic Mercure et écoute côté Twig

Par défaut, le topic est `wrap_notificator/flash/{sessionId}`. Dans votre layout :
...
Si iziToast est chargé dans la page (présence de `window.iziToast`), le bundle affichera automatiquement les notifications (Mercure et Live Flash) via **iziToast**.

Sinon, le rendu retombe sur les **toasts Bootstrap**.

### Flash Symfony "classiques" (sans Mercure)

Si vous ne souhaitez pas utiliser Mercure (ou si `live_flash.enabled=false`), vous pouvez quand même afficher les flash messages Symfony avec le thème WrapNotificator.

Dans votre layout, incluez d'abord les helpers UI :

```twig
{{ wrap_notify_bootstrap() }}
```

Puis affichez les flashes (consommation via `app.flashes`) :
...
{# Désactiver la file (toastQueue) et déclencher directement #}
